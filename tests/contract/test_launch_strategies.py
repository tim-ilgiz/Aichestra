...
def test_discover_launches_prefers_native_then_bridge(monkeypatch):
    runtime = AgentRuntime("codex", True)
    facts = DiscoveryFacts(runtimes=(runtime,))
...
def test_terminal_bridge_rejects_unproven_process(monkeypatch):
    target = _opencode_target()
    adapter = TerminalBridgeLaunch()
...
def test_existing_terminal_requires_attested_handle():
    ...
    # Injected capability may advertise existing-terminal once attested.
